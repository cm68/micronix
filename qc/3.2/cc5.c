...
chkeyword()
	...
	return (  chks("break")   || chks("case")    || chks("continue")
	       ...
	       || chks("struct")  || chks("switch")  || chks("union")
	       || chks("void")	  || chks("while")   || chkdecl());
	}
...
struct parsestack *ts;
	...
	for (p = ts->curr_ptr; --p >= startstack; ) {
		...
		case T_FUNC:
			...
			base.p_type = type;
			type = addtype(T_FUNC, &base, S_FUNC);
			break;
		case T_ARRAY:
			...
			base.p_type = type;
			type = addtype(T_ARRAY, &base, size);
			break;
			}
		}
	return type;
...
char *name;
	...
	if (!chksym(name)) {
		...
		skip();
		keyerr(name);	/* illegal use of keyword */
		return FALSE;
		}
...
register char *name;
	...
	*name = '\0';
	return p;		/* return location of char after name */
	}
...
register char *name;
	...
	if (p = findmac(name)) {	/* already defined? */
		p = *(int *)(p+MACPTR); /* ptr to replacement text */
...
delmac()
	{
	char *p, *isdef();
	if (p = isdef())		/* if the name is defined, */
		*p = '\0';		/* wipe it out */
	else
...
register struct operand *cexp;
	...
	heir3(cexp);		/* no comma operators or assignment */
	genflag = TRUE;
...
struct operand *op;
	...
	while ((c = getstring()) != -1) {
		if (plitpool-litpool < litpsize-1)
			*plitpool++ = c;
		else if (lit_full)
			continue;
		else {
			error("String space full");
			lit_full = TRUE;
			}
		}
	if (plitpool-litpool < litpsize)
		*plitpool++ = '\0';
	return TRUE;
	}
...
char *name;
	{
	struct st *findvar();
	return findvar(STARTGLB, glbptr, name, ID_VAR);
	}
struct st *
findloc(name)
char *name;
	{
	struct st *findvar();
	return findvar(locptr+1, symtab+nsym, name, ID_VAR);
	}
struct st *
findtag(name)
char *name;
	...
	if (p = findvar(locptr+1, symtab+nsym, name, ID_STRUCT))
		return p;
	else
		return findvar(STARTGLB, glbptr, name, ID_STRUCT);
	}

#ifdef PORTABLE

struct st *
findvar(start, end, name, idset)
struct st *start;
...
register idset;
	...
	register struct st *p;
	for (p = start; p < end; ++p){
		if (astreq(name, p->st_name, NAMEMAX)
		    && p->st_idset==idset)
			return p;
		}
	return NULL;
...
struct st *addglb(name,type,sc,idset,info)
register char *name;
register struct typeinfo *type;
	{
	...
	if (idset != ID_STRUCT && (p = findglb(name))) {
		...
		if (p->st_type != type) 	/* types must be the same*/
			errname("Inconsistent declaration", name);
		}
	else if (p = addvar(glbptr,name,type,sc,idset,info))
		++glbptr;
	return p;
	}
struct st *
addloc(name,type,sc,idset,info)
char *name;
struct typeinfo *type;
	{
	struct st *p, *addvar();

	if (p = addvar(locptr,name,type,sc,idset,info))
		--locptr;
	return p;
	}
struct st *
addvar(p,name,type,sc,idset,info)
register struct st *p;
char *name;
register struct typeinfo *type;
	{
	...
	if (type != NULL) {
	 	reftype(type);
		if (type->t_code == T_STRUCT && idset == ID_VAR
		   && type->t_size == 0)
			error("No template");
		}
	strcpy(p->st_name, name);
	p->st_type  = type;
	p->st_sc = sc;
	p->st_idset = idset;
	p->st_info = info;
...
/*
 *	Member table handling functions
 */
/* Add a member to the member table */
struct memtab *
addmem(sym, type, prevmem)
struct st *sym;
struct memtab *prevmem;
	{
	register struct memtab *currmem;
	if (freemem == NULL) {
		error("Member table full");
		return NULL;
		}
	else {
		currmem = freemem;	/* use next free member slot */
		freemem = freemem->nextmem; /* update free list ptr */
		currmem->p_sym = sym;	/* record symbol table entry */
		currmem->nextmem = NULL; /* end of current list */
		if (prevmem == NULL)	/* first member in this struct */
			type->t_base.memlist = currmem;
		else
			prevmem->nextmem = currmem; /* chain members */
		return (currmem);
		}
	}
/* Delete an entire member list */
delmem(memlist)
struct memtab *memlist;
	{
	register struct memtab *p;
	if (freemem != NULL) {	/* add memlist to front of free list */
		for (p = memlist; p->nextmem != NULL; p = p->nextmem)
			;	/* find last member in list */
		p->nextmem = freemem; /* make it point to old free list */
		}
	freemem = memlist;
	}
/*
...
inittypes()
	...
	chartype = addtype(T_CHAR, (union baseinfo *) NULL, S_CHAR);
	reftype(chartype);
	inttype  = addtype(T_INT, (union baseinfo *) NULL, S_INT);
	reftype(inttype);
	unsgntype = addtype(T_UNSIGNED,(union baseinfo *)NULL,S_UNSIGN);
	reftype(unsgntype);
	labeltype = addtype(T_LABEL, (union baseinfo *) NULL, 0);
	reftype(labeltype);
	functype = addtype(T_FUNC, (union baseinfo *) &inttype, 0);
	reftype(functype);
	}
...
struct typeinfo *type;
	...
	return addtype(T_PTR, (union baseinfo *)&type, S_PTR);
	}
...
union baseinfo *base;
	...
	for (p=basetypes[typecode]; p!=NULL; p=p->t_next) {
		if (p->t_size != size)
			continue;
		if (typecode == T_STRUCT) {
			if (p->t_base.memlist == base->memlist)
				break;
			}
		else if (p->t_base.p_type == base->p_type)
			break;
		}
	return p;
	}
...
register union baseinfo *base;
	{
	register struct typeinfo *p, *p_base;

	p_base = (base==NULL || typecode==T_STRUCT) ? NULL: base->p_type;
	if (p_base != NULL)
		reftype(p_base);
	if (basetypes[T_NONE] == NULL && !typecollect()) {
		...
		p->t_next = NULL;
		if (typecode == T_STRUCT)
			p->t_base.memlist = base->memlist;
		else
			p->t_base.p_type = p_base;
	 	return p;
		}
	}
...
{	register struct typeinfo *t, *prev;
	...
	if (p->t_code != T_STRUCT && p->t_base.p_type != NULL)
		unreftype(p->t_base.p_type);
	prev = NULL;
...
typecollect()
	...
	for (code=T_NONE+1; code<T_MAX; ++code) {
		 ...
		 */
			if (code!=T_STRUCT && p->t_base.p_type!=NULL)
				p->t_base.p_type->t_refs--;
			freetype(prev, p);
			++freed;
			}
		}
...
struct operand *op;
	{
	chtype(op, op->op_type->t_base.p_type);
	}
...
struct operand *lop, *rop;
	...
	if (lop->op_type->t_code == T_ARRAY) /* Change array to ptr */
		chtype(lop, ptrto(lop->op_type->t_base.p_type));
	}
chtype(op,new)
register struct operand *op;
...
struct operand *op;
	...
	if (type->t_code == T_PTR)
		...
		return ptrto(type->t_base.p_type);
	else
		return NULL;
	}
islval(op)
