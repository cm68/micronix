...
int		segtype = CODE,	/* record whether code or data segment */
		...
		pregflag = FALSE,	/* does preg contain curr val of
					 * expr? */
...
FILE           *input,		/* iob for current input file */
               ...
               *holdinput[MAXINCL];	/* hold input file iob during
					 * #include */
...
int		ninfils   ,	/* # of input files */
		...
		holdlineno    [MAXINCL],	/* hold input file line no
						 * during #include */
		ifdepth = 0,	/* current depth of #if nesting */
		holdif        [MAXIF],	/* hold process status of nested #if */
		holdelse      [MAXIF],	/* record presence of #else for
					 * nested #if */
		condif = PROCESS,	/* records whether #if says to
					 * PROCESS or SKIP */
		condelse = FALSE,	/* records whether matching #else was
					 * found */
		locspace     ,	/* local space needed in current function */
		retlab       ,	/* label # of return for current function */
		nregvar      ,	/* # of register variables in current
				 * function */
...
char           *peepbuf,	/* buffer for patterns */
		peepsym       [NAMESIZE];	/* name of symbol currently
						 * in preg */
...
/* of arrays > INITSIZE */
		...
		codeflag = DEFASM;	/* default assembler: 'm'-M80,
					 * 'a'-RMAC */
#if DEFASM == 'm'
char		defext    [] = "MAC";
#else
