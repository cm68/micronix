...
char *funcname;
	...
	leaving = (inout == '<');
	if (leaving)
		cpush(PREG);		/* save return value */
	else {
		dataseg();		/* build trace message */
		label = getlabel();
		linelabel(label);
		defbyte();
		outdec(' ');		/* enter (>) or exit (<) */
	...
	printlabel(label);
	nl();
	ldmemi(inout);			/* enter (>) or exit (<) */
	cpush(PREG);
	ccall("puts");			/* print message */
	cpop(PREG);			/* clear arg off stack */
	if (leaving)
		cpop(PREG);		/* restore return value */
	}
/*
 * Do binary operators
...
int (*heirn)();
	...
	(*heirn)(rop);
	if (specialcase(lop, op, rop))
		return FALSE;
	else if ((oprev=loadops(lop, rop)) == CONSTANT)
		return CONSTANT;
	unsignlop = (int)isptr(lop) || lop->op_type->t_code==T_UNSIGNED;
	switch (op) {
	case '|':
		cor();
		break;
...
register struct operand *lop, *rop;
	...
	if (loadops(lop, rop))
		swap_reg();		/* operands loaded in reverse order */
	csub();
	if (rptrtype) {
		if (!lptrtype)
...
register struct st *sym;	/* symbol table entry (or NULL) */
	...
	needpunc(')');
	if (sym) {
		p = sym->st_name;	/* special case printf/scanf */
		if ((q = rindex(p, 'p')) && strcmp(q, "printf")==0 ||
		    (q = rindex(p, 's')) && strcmp(q, "scanf")==0) {
...
register struct operand *op;
	...
	sym = op->op_sym;
	if (peepflag == PREGSYM && strcmp(sym->st_name, peepsym) == 0
	   && op->op_val == peepoffset && op->op_type->t_size == peeplen)
