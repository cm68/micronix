...
static struct buf
	*curbuf = 0,	/* buffer currently under scrutiny */
...
if ((dev & (ALT | TYPE | DOUBSIDE)) != (d->flags & (ALT | TYPE | DOUBSIDE)))
			return djbusy ();
		}
 ...
 */
	...
	d->flags |= ISOPEN | (dev & (ALT | TYPE | DOUBSIDE)); /* set the alternation */
        }
 ...
 */
	...
	if 	(
		inservice /* already working on it */
		||
...
/*		D[ 6] = NOSTAT;		*/
		...
		if	(
			D[4] == OKSTAT		/* read worked */
			&&
			kbuf [1] == 0xff	/* valid dos fat */
			&&
			kbuf [2] == 0xff
			&&
			(*kbuf & 0xfc) == 0xfc	/* valid config. byte */
			)
			{
			if (*kbuf & 1)
				S->config = DOUBSIDE;
