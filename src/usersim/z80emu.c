 ...
 *
 * usersim/z80emu.c
 * Changed: <2022-01-05 12:21:44 curt>
 *
...
static int emulate(Z80_STATE * state,
		   int opcode,
		   int elapsed_cycles, int number_cycles, void *context);

void
Z80Reset(Z80_STATE * state)
{
    int i;
    ...
    for (i = 0; i < 16; i++)
        state->dd_register_table[i]
            = state->fd_register_table[i] = state->register_table[i];
...
int
Z80Interrupt(Z80_STATE * state, int data_on_bus, void *context)
{
...
int
Z80NonMaskableInterrupt(Z80_STATE * state, void *context)
{
    int elapsed_cycles;
...
static int
emulate(Z80_STATE * state,
	int opcode, int elapsed_cycles, int number_cycles, void *context)
{
    int pc, r;
    int d;
    int n;
