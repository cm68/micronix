...
struct pattern bdosa_pats[] = {
	{ 0 }
};
struct pattern bdoshl_pats[] = {
		{ "close_1", &a_close1, -0x45, {
			0xcd, ANY, ANY,
			0xe5,
			0x06, 0x08,
			0xdd, 0x7e, 0x06,
	...
	findall(addr_t addr, struct pattern *ptab)
	{
		struct pattern *p;
		int i;
		...
		printf("search for all calls to %x\n", addr);

		/*
		 * let's find all the bdos calls and classify them
		 */
		for (i = 0x100; (i = next_callto(i, addr)) != 0xffff; i += 3) {
			p = lookup_pat(i, ptab);
			if (p) {
	...
	process(char *fn)
	{
		 ...
		 * let's find all the bdos calls and classify them
		 */
		findall(0x0005, entry_pats);

		/*
		 * now, let's find all the calls to the ones we just found
		 */
		findall(a_bdos, bdos_pats);
		findall(a_bdosa, bdosa_pats);
		findall(a_bdoshl, bdoshl_pats);
	}
