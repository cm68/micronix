 ...
 * 2.44 of 5/26/77
 *
 * Ported to micronix from extra/pwb/spencer.  See NOTES beside this
 * file for what the port changed: the v6 initializer syntax was
 * normalised to K&R, the statb struct mapped to sys/stat.h, and the
 * libPW login helpers reduced to libpw.c.
 */
...
#include <sys/fs.h>
#include <sys/stat.h>
/* the PWB names for the two mode bits the shell tests */
#define	IFMT	S_IFMT
...
long tell();
char *logtty(), *logname(), *logdir(), *nxtarg(), *sname();
...
e3() {
	...
	if (eq(a, "-f")) {
		if (stat(p1, &sb) == -1)
			return 0;
		return ((sb.st_mode & IFMT) == 0);
	}
	if (eq(a, "-d")) {
		if (stat(p1, &sb) == -1)
			return 0;
		return ((sb.st_mode & IFMT) == IFDIR);
	}
	if (eq(a, "-s")) {
		if (stat(p1, &sb) == -1)
			return 0;
		return (sb.st_size0 || sb.st_size1);
	}
