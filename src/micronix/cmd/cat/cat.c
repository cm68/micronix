 ...
 * Concatenate files.
 *
 * 2.11BSD cat (cat.c 5.2, Berkeley 12/6/85), ported to micronix.
 *
 * cmd/cat/cat.c
 *
 * The port is small: the micronix inode has no st_blksize, so the
 * block-size sniffing is gone and fastcat reads BUFSIZ at a time,
 * which on this machine is 512 - a disk block, which is what
 * st_blksize would have said anyway.  Everything else - the option
 * flags, the "input is output" check by device and inode, the
 * multiple-write loop for pipes - is as Berkeley wrote it.
 *
...
char **argv;
{
	...
	while (--argc > 0) {
		...
		if (fi!=stdin)
			...
			/*
			 * reset sticky eof.  clearerr is not in this
			 * stdio; the flag bits are, and this is all
			 * the function ever was.
			 */
			fi->_flag &= ~(_IOEOF|_IOERR);
		if (ferror(stdout)) {
