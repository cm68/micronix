...
get1core(){
	register int i, val;
	register char *p;
ccptr =	ccl = (char *)lxalloc(CCLSIZE,sizeof(*ccl));
def = (char **)lxalloc(DEFSIZE,sizeof(*def));
subs = (char **)lxalloc(DEFSIZE,sizeof(*subs));
dp =	dchar = (char *)lxalloc(DEFCHAR,sizeof(*dchar));
sname = (char **)lxalloc(STARTSIZE,sizeof(*sname));
sp = 	schar = (char *)lxalloc(STARTCHAR,sizeof(*schar));
...
get2core(){
	register int i, val;
	register char *p;
gotof = (int *)lxalloc(nstates,sizeof(*gotof));
nexts = (int *)lxalloc(ntrans,sizeof(*nexts));
nchar = (char *)lxalloc(ntrans,sizeof(*nchar));
state = (int **)lxalloc(nstates,sizeof(*state));
atable = (int *)lxalloc(nstates,sizeof(*atable));
sfall = (int *)lxalloc(nstates,sizeof(*sfall));
cpackflg = (char *)lxalloc(nstates,sizeof(*cpackflg));
tmpstat = (char *)lxalloc(tptr+1,sizeof(*tmpstat));
foll = (int **)lxalloc(tptr+1,sizeof(*foll));
nxtpos = positions = (int *)lxalloc(maxpos,sizeof(*positions));
...
get3core(){
	register int i, val;
	register char *p;
verify = (int *)lxalloc(outsize,sizeof(*verify));
advance = (int *)lxalloc(outsize,sizeof(*advance));
stoff = (int *)lxalloc(stnum+2,sizeof(*stoff));
