static char *sccsid = "@(#) escape.c v1.1p4 90/01/15 rosenkra\0            ";
 ...
 */
	...
	while (*s != EOS)
	{
		...
		else if (*(s + 1) == 'f')
		{
			...
			/*
			 *  it parses 1 char of s and returns esc seq for
			 *  \fB, \fS, \fI, and \fR
			 */
			fontchange (*s, pfs);

			/*
			 *   imbed the atari vt52 or ANSI escape seq
			 */
			while (*pfs)
				*t++ = *pfs++;
			++s;			/* skip B,I,R,S,P */
		...
		else if (*(s + 1) == '(')
		{
			...
			if (ansi_escapes)
			{
				...
				/*
				 *   skip proper num char in s and add c to
				 *   target
				 */
				if (inc)
				{
					...
					/*
					 *   imbed special char escape seq.
					 *   we should put out something like:
					 *
					 *	ESC [ 9 m c ESC [ m
					 *
					 *   "c" is the ascii char in the
					 *   alt char set. we return to normal
					 *   mode (font) immediately so if u
					 *   want to continue bold, you will
					 *   have to do another \fB. alt char
					 *   set is mutually exclusive.
					 */
					strcpy (t, s_special);	/* start...*/
					t   += strlen (s_special);

					*t++ = c;		/* the char */

					strcpy (t, e_special);	/* ...end */
					t   += strlen (e_special);
				}
			}
			else
 ...
 */
	...
	/*
	 *   special symbols (alt char set and std font)
	 */
	if (ansi_escapes)
	{
		return (dev_char (c1, c2, c));
	}
	else
	{

	/*
	 *   not ansi, just use whatever is in the standard set...
	 *
	 *   special chars
	 */
	...
	/*
	 *   if we get here, we do no support the escape. send back a space.
	 *   caller will push the 2 char escape sequence to output.
	 */
	*c = ' ';
	...
	case 'P':				/* previous (exchange) */
		if (dc.dofnt == YES)
		{
			if (ansi_escapes)
			{
				...
				else if (dc.lastfnt == 4)
					strcpy (s, s_special);	/* to S */
				else
					*s = '\0';		/* nothing */
			}
			else
