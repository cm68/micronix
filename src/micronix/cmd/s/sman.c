...
#define MAXCOLS		120     /* maximum possible screen width */
                                /*
                                 * size of buffers for expanded text 
                                 */
#define MAXEXPAND	MAXTEXT+100
#define MAXROWS		60      /* maximum possible screen height */
                                /*
...
s_init()
{
    ...
    /*
     * allocate storage for remembering screen contents 
     */
    msg_save = ckalloc(ncols + 1);
    id = (int *) ckalloc(sizeof(int) * (nrows + 1));
    text = (char **) ckalloc(sizeof(char *) * (nrows + 1));
...
bottom()
{
    ...
    ptoseg(cur_text, strlen(cur_text) - 1, &last_seg, &junk);
     ...
     */
    ptoseg(cur_text, cur_pos, &cur_seg, &cur_col);
...
changes()
{
    ...
    for (row = 1, line = new_first;
        row <= nrows && (row <= last_row || line == cur_line)
        && line <= last_line; row = after_line(row), ++line) {
        ...
        } else if (row < useful_row || useful_row == 0  /* no more useful
                                                         * rows */
 ...
 */
displ_text(row, line_id, nsegs, s)
...
good_first()
{
    int best_first, cur_line, cur_pos, first, last, line, max_overlap,
        overlap;
...
/*
 * ptoseg - convert a line position to a screen segment and column 
 */
ptoseg(t, pos, seg_ptr, col_ptr)
    char *t;
...
repaint()
{
    ...
    for (row = 1, line = first_line = max(cur_line - 8, 1);
        row <= last_row && line <= b_size(); row = after_line(row), ++line)
        display(row, line);
    bottom();
}
 ...
 */
repl_text(row, line_id, new_segs, useful_row, new_text)
    ...
    char *new_text;             /* text of the new line */
{
     ...
     */
    for (p1 = old_text, p2 = new_text; *p1 != '\0' && *p1 == *p2; ++p1, ++p2);
     ...
     */
    d_count = s1 - p1 - (s2 - p2);      /* counts deleted chars (<0 for
                                         * insert) */
 ...
 */
scroll(k, line)
    int k,                      /* number of lines to be pushed off the top
                                 * of the screen */
