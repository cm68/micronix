    ...
    char c, op;
{
    ...
    case 'w':
        ...
        if (op != ' ') {
            b_getcur(&new_line, &new_pos);
            if (new_line != cur_line || new_pos == cur_pos) {   /* last word
                                                                 * in buffer */
                /*
...
/*
 * -------------------- the j and k commands -------------------- * * Entry
 * point: * * do_up_down(i) * int i; * Move the cursor i lines, staying in
 * the same column. Throughout * an uninterrupted sequence of j and k
 * commands, the cursor stays * in the same column subject to the constraint
 * that it always lie * on a buffer character. 
 */

do_up_down(i)
    int i;
{
    static int col = 0;         /* remembered column */
...
/*
 * ptocol - convert a line position to a screen column 
 */
int
ptocol(line, pos)
    int line, pos;
 ...
 * Entry point: * * loc_char(ch, way) * char ch; * int way; * Set cursor to
 * the next position of ch in the current line. * If way = 1, then search to
 * the right; otherwise, way = -1 and * the search moves left. If ch = '\0',
 * then ch and way are taken * from the previous call to loc_char. 
 */
...
/*
 * -------------------- the b and w commands -------------------- * * Entry
 * point: * * loc_word(way) * int way; * Set cursor to the start of the next
 * word.  If way = 1, then * search toward file's end; otherwise, search
 * toward file's * beginning. Do not "wrap around" at the ends of the file. 
 */
...
/*
 * -------------------- the /, \ and n commands -------------------- * *
 * Entry point: * * loc_string(ch) * char ch; * Set cursor to the next
 * instance of a user-supplied string. * (Loc_string prompts the user to
 * provide the string.) * If ch = '/', then the search is forward in the file 
 * and * wraps around from the end of the file to the start.  If * ch = '\',
 * then the search is backward (toward the start of * the file) and wraps
 * around from the first line to the last line. * For commands /<return> and
 * \<return> the string is taken * from the previous call to loc_string.  If
 * ch = '\0', then the * string and way are taken from the previous call to
 * loc_string. 
 */
...
loc_string(ch)
    char ch;
{
    static int way = 0;         /* remembered direction */
    static char string[MAXTEXT - 1] = 0;        /* remembered pattern */
