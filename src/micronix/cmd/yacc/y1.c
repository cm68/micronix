...
/*
 * The tables were fixed worst-case arrays sized by the _xxx macros in
 * dextern, which left a small grammar carrying a 28k data segment it
 * never touched.  Two passes now: sizes() reads the grammar once
 * against generous temporary tables to count terminals, nonterminals
 * and productions, then mallocs the real tables to just what that
 * grammar needs; setup() reads it a second time and fills them.
 */

allocgrammar() {
	mem0 = (int *)calloc( memsiz, sizeof(int) );
	mem = mem0;
	prdptr = (int **)calloc( prdlim, sizeof(int *) );
	levprd = (int *)calloc( prdlim, sizeof(int) );
	trmset = (struct sxxx1 *)calloc( tlim+1, sizeof(struct sxxx1) );
	nontrst = (struct sxxx2 *)calloc( ntlim+1, sizeof(struct sxxx2) );
	cnames = malloc( cnamsz + 1 );
	trmlev = (int *)calloc( tlim+1, sizeof(int) );
	}

alloclr() {
	pstate = (struct item **)calloc( stsize+2, sizeof(struct item *) );
	tystate = (int *)calloc( stsize+1, sizeof(int) );
	amem = (int *)calloc( actsiz+1, sizeof(int) );
	memp = amem;
	temp1 = (int *)calloc( tlim + ntlim + stsize + 4, sizeof(int) );
	indgo = (int *)calloc( stsize+1, sizeof(int) );
	defact = (int *)calloc( stsize+1, sizeof(int) );
	tstates = (int *)calloc( tlim+1, sizeof(int) );
	ntstates = (int *)calloc( ntlim+1, sizeof(int) );
	mstates = (int *)calloc( stsize+1, sizeof(int) );
	lkst = (struct looksets *)calloc( lsetsz+1, sizeof(struct looksets) );
	wsets = (struct wset *)calloc( wssize+1, sizeof(struct wset) );
	zzcwp = wsets;
	cwp = wsets;

	/* the optimizer's aliases into the tables */
	ggreed = (int *)lkst;
	pgo = (int *)wsets;
	yypgo = (int *)nontrst;
	pmem = mem0;
...
freegrammar() {
	free(mem0); free(prdptr); free(levprd); free(trmset); free(nontrst);
	free(cnames); free(trmlev);
	}

freelr() {
	free(pstate); free(tystate); free(amem); free(temp1); free(indgo);
	free(defact); free(tstates); free(ntstates); free(mstates); free(lkst);
	free(wsets);
	}

sizes(argc, argv) int argc; char *argv[]; {
	int nt, nnt, np, mc, cc;

	/* pass 1: a small fixed measuring buffer.  setup() reads the
	   grammar into it just to count terminals, nonterminals and
	   productions; anything bigger than this is a grammar this
	   machine could not have run anyway, and it fails here with the
	   usual "too many ..." complaints rather than overflowing. */
	tlim = ntlim = prdlim = 64;
	memsiz = 512;
	cnamsz = 512;
	allocgrammar();
	setup( argc, argv );
	...
	if( NWORDS(nt) > _tbitset )
		error( "too many terminals for lookahead set" );

	/* pass 2: what was counted, plus bounds for the LR tables */
	tlim = nt + 2;
	ntlim = nnt + 2;
	prdlim = np + 2;
	...
	/* setup() reads the grammar again; start the counters fresh */
	nterms = 0;
	nnonter = -1;
	nprod = 1;
	ndefout = 3;
...
main(argc,argv) int argc; char *argv[]; {
	...
	sizes(argc,argv); /* pass 1: count the grammar, size the tables */
	setup(argc,argv); /* pass 2: initialize and read productions */
