		...
		TLOOP(p0){
			if( (p1=temp1[p0])!=0 ) {
				if( p1 < 0 ){
					...
					goto exc;
				exc:
					if( flag++ == 0 ) fprintf( ftable, excfsb[lflag], i );
					else if( lflag == RATFOR ) fprintf( ftable, "else " );
...
carray( s, v, n ) char *s; int *v, n; {

	register i;

	fprintf( ftable, "int %s[] = {\n", s );
	for( i=0; i<n; ){
...
callopt(){
	...
	if( ftable == NULL ) error( "cannot reopen output file" );
