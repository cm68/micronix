 ...
 *  2+head+2+page[56]+5
 *
 * 4.3BSD pr (pr.c 4.5, Berkeley 12/12/84), ported to micronix.
 *
 * One thing came out: pr chmod'ed the terminal to 0600 while it
 * printed, so nobody could write(1) into the listing, and put the
 * mode back on the way out.  That wanted ttyname(), which this libc
 * does not have, so the port prints without the courtesy - tty stays
 * null and the done/onintr pair have only the exit left to do.
 *
 * Everything else is stdio, stat for the header's date - d_mtime
 * through the st_mtime macro, and ctime() on it - and column math
 * that fits in the 16 bits an int is here.
 *
