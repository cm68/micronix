 ...
 * cmd/diff/diffreg.c
 *
 * Ported from 2.11BSD usr/src/bin/diff/diffreg.c.  Two things changed:
 * -h no longer execs diffh (which is not ported) and falls through to
 * the full algorithm, and asciifile() no longer checks the a.out magic
 * - micronix's object format is not a.out, so only the high-bit test
 * decides whether a file is text.
 */
