/*
 * diffh - the halfhearted diff, run by diff -h
 *
 * cmd/diff/diffh.c
 *
 * Ported from 2.11BSD usr/src/bin/diff/diffh.c.  It buffers at most
 * RANGE lines of each file (LEN bytes each), so its memory is fixed
 * rather than growing with the input - which is what makes -h the way
 * to diff something too big for diffreg's Stone algorithm.  The only
 * porting change is the includes: micronix has types.h rather than
 * sys/types.h, and sys/stat.h embeds struct dsknod so sys/fs.h has to
 * come first.
 */
#include <stdio.h>
#include <ctype.h>
