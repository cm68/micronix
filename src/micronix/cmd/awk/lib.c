...
#define	FINIT	{EMPTY, EMPTY, 0L, FLD|STR}
cell fldtab[MAXFLD] = {	/*room for fields */
	{ "$record", record, 0L, STR|FLD},
	...
	while (svargc > 0) {
		...
		*rr = 0;
		...
		if (c != EOF || rr > record) {	/* normal record */
			recloc->tval &= ~NUM;
			recloc->tval |= STR;
			nrloc->fval = fadd(nrloc->fval, itof(1));
			nrloc->tval &= ~STR;
			nrloc->tval |= NUM;
			return(1);
		}
...
char *s;
{
	...
	for (p=s; *p != '='; p++)
		;
	*p++ = 0;
	q = setsymtab(s, tostring(p), 0L, STR, symtab);
	setsval(q, p);
	...
	*fr = 0;
	...
	for(i=1; i<=maxfld; i++)
		if(isnumber(fldtab[i].sval)) {
			fldtab[i].fval = fatof(fldtab[i].sval);
			fldtab[i].tval |= NUM;
		}
	setfval(lookup("NF", symtab, 0), itof(maxfld));
	if (dbg)
		for (i = 0; i <= maxfld; i++)
			printf("field %d: |%s|\n", i, fldtab[i].sval);
}
...
error(f, s, a1, a2, a3, a4, a5, a6, a7) {
	...
	if (NR && fcmp(*NR, 0L) > 0) {
		char nb[20];
		fprintf(stderr, " record number %s\n", ftoa(nb, *NR, 0));
	}
	if (f)
		exit(2);
}
