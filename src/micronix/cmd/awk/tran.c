...
syminit()
{
	setsymtab("0", tostring("0"), 0L, NUM|STR|CON|FLD, symtab);
	/* this one is used for if(x)... tests: */
	setsymtab("$zero&null", tostring(""), 0L, NUM|STR|CON|FLD, symtab);
	recloc = setsymtab("$record", record, 0L, STR|FLD, symtab);
	dprintf("recloc %o lookup %o\n", recloc, lookup("$record", symtab, 0), NULL);
	FS = &setsymtab("FS", tostring(" "), 0L, STR|FLD, symtab)->sval;
	RS = &setsymtab("RS", tostring("\n"), 0L, STR|FLD, symtab)->sval;
	OFS = &setsymtab("OFS", tostring(" "), 0L, STR|FLD, symtab)->sval;
	ORS = &setsymtab("ORS", tostring("\n"), 0L, STR|FLD, symtab)->sval;
	OFMT = &setsymtab("OFMT", tostring("%.6g"), 0L, STR|FLD, symtab)->sval;
	FILENAME = &setsymtab("FILENAME", EMPTY, 0L, STR|FLD, symtab)->sval;
	nfloc = setsymtab("NF", EMPTY, 0L, NUM, symtab);
	NF = &nfloc->fval;
	nrloc = setsymtab("NR", EMPTY, 0L, NUM, symtab);
	NR = &nrloc->fval;
}
...
register cell *vp;
{
	...
	if ((vp->tval & NUM) == 0) {
		...
		if (isnumber(vp->sval)) {
			vp->fval = fatof(vp->sval);
			if (!(vp->tval & CON))	/* don't change type of a constant */
				vp->tval |= NUM;
		}
		else
			vp->fval = 0L;	/* not a very good idea */
	}
	dprintf("  %g\n", vp->fval, NULL, NULL);
	return(vp->fval);
}
...
register cell *vp;
{
	...
	if ((vp->tval & STR) == 0) {
		if (!(vp->tval&FLD))
			strfree(vp->sval);
		if (fcmp(vp->fval, itof(ftrunc(vp->fval))) == 0)
			ftoa(s, vp->fval, 0);
		else
			ftoa(s, vp->fval, 6);
		vp->sval = tostring(s);
