...
break;
case 30:
	{ mustfld=1; yylval = (hack)setsymtab(yytext, EMPTY, 0L, NUM, symtab); RETURN(VAR); }
break;
case 31:
{
		yylval = (hack)setsymtab(yytext, EMPTY, fatof(yytext), CON|NUM, symtab); RETURN(NUMBER); }
break;
case 32:
...
break;
case 57:
{ yylval = (hack)setsymtab(yytext, tostring(""), 0L, STR|NUM, symtab); RETURN(VAR); }
break;
case 58:
	{ BEGIN str; clen=0; }
break;
...
case 79:
	{ char *s; BEGIN A; cbuf[clen]=0; s = tostring(cbuf);
		...
		yylval = (hack)setsymtab(cbuf, s, 0L, CON|STR, symtab); RETURN(STRING); }
break;
case 80:
