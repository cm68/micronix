...
#include "less.h"
/*
 * sys/signal.h, not signal.h: the compiler's own signal.h is
 * CP/M's unless something defines unix, and the micronix numbers
 * are the ones the kernel answers to.
 */
...
#endif
	/*
	 * Force standard input to be the terminal.  The original
	 * opened /dev/tty; micronix has no controlling-terminal
	 * device, but file descriptor 2 leads to the keyboard even
	 * in a pipeline - it is where ttyin.c reads commands - so
	 * the child gets a duplicate of that.
	 */
	inp = dup(0);
	close(0);
	dup(2);
