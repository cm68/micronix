 ...
 * Output a character.
 *
 * Micronix raw mode is v6 RAW: it takes the output cooking with it,
 * so nothing maps newline to return-newline but us.  This is the
 * one place every character passes through, so this is the place.
 */
	public void
putc(c)
	int c;
...
/*
 * Was return_to_continue, which is one character past what the
 * compiler keeps of a name.
 */
static char press_return[] = "  (press RETURN)";
	...
	char *s;
{
	...
	if (strlen(s) + sizeof(press_return) + 
...
error_width()
{
	 ...
	 */
	return (sc_width - 
		(sizeof(press_return) + so_width + se_width + 1));
}
#endif
