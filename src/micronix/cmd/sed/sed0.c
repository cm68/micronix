/*
 * sed -- stream editor: option parsing and script compilation
 *
 * 2.11BSD sed (sed0.c 4.2, 85/06/19), ported to micronix.  What
 * changed and why is in the comment at the top of sed.h; the code
 * below is the original except that the globals sed.h used to
 * define are defined here, and the reptr union reads through u.
 *
 * cmd/sed/sed0.c
 *
...
/*
 * the globals, declared in sed.h and living here - see the note
 * there.  All start zero, so none carries an initializer.
 */
FILE	*fin;
...
char	*expbuf;
{
	...
	/*
	 * The original counted a char up until its sign bit came on -
	 * "for(c = 0; !(c & 0200); c++)" - which leans on 8-bit
	 * wraparound and is an expression ccc has no code for.  An int
	 * index bounded by 0200 walks the same 128 slots and says what
	 * it means.
	 */
	{
		register int	i;
