/*
 * sed -- stream editor: execution
 *
 * 2.11BSD sed (sed1.c 4.2, 85/04/05), ported to micronix.  What
 * changed and why is in the comment at the top of sed.h; the code
 * below is the original except that the reptr union reads through u,
 * and the two trans[] entries for backspace and tab spell their
 * embedded backspace as \010 - the original held the raw byte,
 * which ccc's preprocessor refuses.  Same bytes in the program.
 *
 * cmd/sed/sed1.c
 *
