 ...
 *
 * usage:  od [-abBcdDhHiIlLopPs[n]vw[n]xX] [file] [[+]offset[.][b] [label]]
 *
 ...
 *
 * 2.11BSD od (od.c 5.13, 2.11BSD 2001/11/19), ported to micronix.
 *
 * cmd/od/od.c
 *
 * What the port took out: the e, f and F conversions.  There is no
 * float type on this machine - libc's %e is a stub that prints '?',
 * for the reason doprnt.c gives - so the flags say so and exit
 * instead of dumping question marks that look like data.
 *
 * And what it had to say differently: bcmp is memcmp with the answer
 * inverted, and the library has only memcmp; -w takes any width and
 * fread was pointed at a 512 byte buffer regardless, so the width is
 * clamped to the buffer here rather than letting a long line dump
 * scribble past it; and the headers are the tree's - types.h and
 * sys/fs.h ahead of sys/stat.h, which needs the dsknod they declare.
 *
...
char	**argv;
{
	...
	if(argc > 0)
	{
		...
		if(*p == '-')
		{
			while(*++p != '\0')
			{
				...
				case 'f':
					fprintf(stderr,
					    "od: no floating point on this machine\n");
					exit(1);
				case 'h':
				case 'x':
					d = &u_s_hex;
				...
				case 'w':
					...
					if (dbuf_size == 0)
						dbuf_size = BIG_DBUF;
					/*
					 * fread fills dbuf regardless of what
					 * -w asked for, so the width cannot
					 * exceed it
					 */
					if (dbuf_size > DBUF_SIZE)
						dbuf_size = DBUF_SIZE;
					continue;
				case 'v':
					showall = YES;
	...
	*cv = (struct dfmt *)0;
	...
	{
		if (same>=0 && memcmp(dbuf, lastdbuf, dbuf_size) == 0 && !showall)
		{
			if (same==0)
			{
				printf("*\n");
...
register char *s;
{
	...
	{
		/*
		 * the space before == is load bearing: the compiler's
		 * lexer misreads the run "++==" as an assignment and
		 * then wants an lvalue.  "*p++ == c" scans correctly.
		 */
		if (*p++ == '.')
			addr_base = 10;
	}
