 ...
 *
 * this calls into the 1990, 8250 and 8250 drivers
 */
...
/*
 * daisy wheel interface unimplemented
 */
static byte
rd_daisy(portaddr p)
{
    ...
    ret = 0;
    if (trace & trace_multio) 
        printf("multio: read daisy %02x = 0x%02x\n", p, ret);
    return ret;
}
...
{
    if (trace & trace_multio) 
        printf("multio: write daisy0 %02x\n", v);
}
...
{
    if (trace & trace_multio) 
        printf("multio: write daisy1 %02x\n", v);
}
...
{
    if (trace & trace_multio) 
        printf("multio: read to %02x+%02x undefined\n", 
...
{
    if (trace & trace_multio) 
        printf("multio: write 0x%02x to undefined %02x+%02x\n", 
...
multio_select(portaddr p, byte v)
{
    ...
    case GRP0:  // parallel port, clock, pic
        activate_1990(p + 2);
        activate_8259(p + 4);
        register_input(p + 0, &rd_daisy);
        register_input(p + 1, &undef_inreg);
        register_input(p + 3, &undef_inreg);
        register_input(p + 6, &undef_inreg);

        register_output(p + 0, &wr_daisy0);
        register_output(p + 1, &wr_daisy1);
        register_output(p + 3, &undef_outreg);
        register_output(p + 6, &undef_outreg);
        break; 
...
/*
 * make a single instance of the multio.
 * there is an ability to have 2 of them, where it does a cascading
 * 8259, but that is left for another day
 */
void
create_multio(portaddr p)
{
    int i;
    int tbase = 0;

    if (p != PORT_MULTIO_MASTER) {
        tbase = 3;
        warn("slave multio not working");
        return;
    }
    for (i = 0; i < 3 ; i++) {
        sprintf(lname, "line%d", tbase + i);
        create_8250(lname, tbase + i, p, vi_3 + i);
    }
    create_1990(p + 2, vi_7);
    create_8259(p + 4);
	register_output(p + 7, &multio_select);
}
